...
if st.session_state.logged_in:
  ...
  if menu == "Logout":
    ...
    if st.session_state.user_role == "SuperAdmin":
      ...
      with tab_set:
        ...
        if settings[8]:
          ...
          if st.button("🗑️ Delete Home Banner"):
            run_query("UPDATE admin_settings SET home_banner=NULL WHERE id=1")
            st.rerun()
      ...
      if st.session_state.user_role == "Wholesaler":
        tab_dash, tab_stock, tab_purch, tab_sales, tab_hist, tab_gst_rep, tab_med_ws, tab_net, tab_prof = st.tabs(
            [
                "📈 Dash",
                "📦 Stock",
                "📥 Purchase",
        ...
        tab_dash, tab_stock, tab_purch, tab_sales, tab_hist, tab_gst_rep, tab_med_st, tab_prof = st.tabs(
            [
                "📈 Dash",
                "📦 Stock",
                "📥 Purchase",
      ...
      with tab_purch:
        ...
        if (
          ...
          if existing_item:
            run_query(
                "UPDATE inventory SET stock = stock + ?, purchase_price=?, selling_price=? WHERE barcode=? AND shop_email=?",
                (i_qty, i_pprice, i_sprice, i_bcode, st.session_state.user_email),
            )
          else:
            run_query(
                "INSERT INTO inventory (shop_email, item_name, purchase_price, selling_price, stock, gst_rate, barcode) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (
                    st.session_state.user_email,
                    i_name,
                    i_pprice,
          ...
          run_query(
              """INSERT INTO transactions (shop_email, date, item_name, qty, total_price, profit, is_gst, trans_type, rate, gst_pct, gst_amt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
              (
                  st.session_state.user_email,
                  str(date.today()),
                  i_name,
        ...
        today_purchases = run_query(
            "SELECT item_name, qty, rate, gst_pct, gst_amt, total_price FROM transactions WHERE shop_email=? AND date=? AND trans_type='Purchase'",
            (st.session_state.user_email, str(date.today())),
        )
        if today_purchases:
          st.dataframe(
              pd.DataFrame(
                  today_purchases,
                  columns=["Item", "Qty", "Buy Rate", "GST %", "GST Amt", "Total"],
              ),
              use_container_width=True,
          )
          if st.button("🖨️ Print Daily Purchase Report"):
            st.session_state.purch_print = generate_purchase_report_html(
                shop_name, str(date.today()), today_purchases
            )
            st.rerun()
        if "purch_print" in st.session_state:
          components.html(st.session_state.purch_print, height=600)
          if st.button("❌ Close Purchase Print"):
            del st.session_state.purch_print
            st.rerun()
      ...
      with tab_sales:
        ...
        if "print_receipt" in st.session_state:
          ...
          with st.expander("👤 Customer Details", expanded=False):
            c1, c2 = st.columns(2)
            with c1:
              raw_c_name = st.text_input("Customer Name")
              cust_name = str(raw_c_name).strip().upper()
            with c2:
              cust_mob = st.text_input("Mobile Number")

          scan_code = st.text_input(
              "🔍 SCAN BARCODE HERE (Use Machine)...", key="s_scan"
          )
          stock_items_pos = run_query(
              "SELECT id, item_name, selling_price, stock, purchase_price,"
              " gst_rate, barcode FROM inventory WHERE shop_email=? AND stock >"
              " 0",
              (st.session_state.user_email,),
          )
          if stock_items_pos:
            item_dict_pos = {
                f"{str(item[1]).upper()} - ₹{item[2]} (Stock: {item[3]})": item
                for item in stock_items_pos
            }
            default_index_pos = 0
            if scan_code:
              for idx, item in enumerate(stock_items_pos):
                if str(item[6]) == str(scan_code):
                  default_index_pos = idx
                  st.success(f"Barcode Matched: {str(item[1]).upper()}")
                  break
            sel_item_pos = st.selectbox(
                "Select Product",
                list(item_dict_pos.keys()),
            ...
            (
                ...
                def_gst,
                _,
            ) = item_dict_pos[sel_item_pos]
            i_name = str(i_name_raw).upper()

            col1, col2, col3 = st.columns(3)
            with col1:
              s_qty = st.number_input(
                  "Quantity", min_value=1, max_value=i_stock, value=1
              )
            with col2:
              s_price = st.number_input(
                  "Rate (₹)", value=float(default_sprice)
              )
            with col3:
              gst_options = {
                  "No GST (0%)": 0,
              ...
              s_gst_pct = gst_options[
                  st.selectbox("Select GST Slab", list(gst_options.keys()))
              ]
            ...
            st.write(
                f"**Base Amount:** ₹{s_base:.2f} | **GST Amount:**"
                f" ₹{gst_amt:.2f}"
            )
            ...
            if st.button("🛒 Generate Sale Bill & Print", type="primary"):
              if s_qty <= i_stock:
                ...
                run_query(
                    """INSERT INTO transactions (shop_email, date, item_name, qty, total_price, profit, is_gst, trans_type, customer_name, customer_mobile, invoice_no, rate, gst_pct, gst_amt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                    (
                        st.session_state.user_email,
                        str(date.today()),
                        i_name,
      ...
      with tab_hist:
        ...
        history = run_query(
            "SELECT invoice_no, date, customer_name, item_name, qty, rate,"
            " gst_pct, total_price, gst_amt, customer_mobile FROM transactions"
            " WHERE shop_email=? AND trans_type='Sale' ORDER BY id DESC LIMIT"
            " 50",
            (st.session_state.user_email,),
        )
        if history:
          ...
          if history_clean:
            ...
            with c1:
              sel_inv = st.selectbox(
                  "🔍 Select Invoice to Reprint",
                  [h[0] for h in history_clean],
              )
            with c2:
      ...
      with tab_gst_rep:
        ...
        gst_data = run_query(
            "SELECT invoice_no, date, customer_name, total_price - gst_amt,"
            " gst_pct, gst_amt, total_price FROM transactions WHERE"
            " shop_email=? AND trans_type='Sale' AND is_gst=1",
            (st.session_state.user_email,),
        )
        if gst_data:
          df_gst = pd.DataFrame(
          ...
          components.html(
              generate_gst_report_html(df_gst, tot_gst, shop_name), height=100
          )
        else:
      ...
      if st.session_state.user_role == "Wholesaler":
        with tab_med_ws:
          st.subheader(
              "🏢 Medical Wholesaler Management (Box & Strip System)"
          )
          with st.form("med_ws_form"):
            mw_name = st.text_input("Medicine Name (Wholesale)")
            mw_boxes = st.number_input("Box Count", min_value=0, value=10)
            mw_s_box = st.number_input(
                "Strips per Box (e.g. 20/200)", min_value=1, value=200
            )
            mw_t_strip = st.number_input(
                "Tablets per Strip", min_value=1, value=10
            )
            mw_p_box = st.number_input(
                "Purchase Price per Box (₹)", min_value=0.0, value=1000.0
            )
            mw_s_box_p = st.number_input(
                "Selling Price per Box (₹)", min_value=0.0, value=1200.0
            )
            mw_gst = st.selectbox("GST Rate (%)", [0.0, 5.0, 12.0, 18.0])

            if st.form_submit_button("Save Wholesale Medicine"):
              if mw_name:
                run_query(
                    """INSERT INTO medical_wholesaler (shop_email, item_name, box_count, strips_per_box, tablets_per_strip, purchase_price_box, selling_price_box, gst_rate, updated_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                    (
                        st.session_state.user_email,
                        mw_name.strip().upper(),
                        mw_boxes,
                ...
                st.success("✅ Wholesale medicine added successfully!")
                st.rerun()

          st.markdown("### 📋 Wholesaler Medicine Inventory")
          mw_data = run_query(
              "SELECT item_name, box_count, strips_per_box, tablets_per_strip,"
              " purchase_price_box, selling_price_box, gst_rate FROM"
              " medical_wholesaler WHERE shop_email=?",
              (st.session_state.user_email,),
          )
          if mw_data:
            st.dataframe(
            ...
            st.info("No wholesale medicines added yet.")

        with tab_net:
          st.subheader("🏪 Live Retailer Stock Tracking")
          r_stocks = run_query(
              "SELECT UPPER(u.name), u.email, UPPER(i.item_name), i.stock,"
              " i.selling_price FROM inventory i JOIN users u ON i.shop_email ="
              " u.email WHERE u.role = 'Shop' AND i.stock > 0"
          )
          if r_stocks:
            st.dataframe(
                pd.DataFrame(
      ...
      if st.session_state.user_role == "Shop":
        with tab_med_st:
          ...
          with st.form("med_st_form"):
            ms_name = st.text_input("Medicine Name (Store)")
            ms_strips = st.number_input("Strips Count", min_value=0, value=50)
            ms_t_strip = st.number_input(
                "Tablets per Strip", min_value=1, value=10
            )
            ms_p_strip = st.number_input(
                "Purchase Price per Strip (₹)", min_value=0.0, value=40.0
            )
            ms_s_strip = st.number_input(
                "Selling Price per Strip (₹)", min_value=0.0, value=60.0
            )
            ms_gst = st.selectbox("GST Rate (%)", [0.0, 5.0, 12.0, 18.0])

            if st.form_submit_button("Save Store Medicine"):
              if ms_name:
                run_query(
                    """INSERT INTO medical_store (shop_email, item_name, strips_count, tablets_per_strip, purchase_price_strip, selling_price_strip, gst_rate, updated_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
                    (
                        st.session_state.user_email,
                        ms_name.strip().upper(),
                        ms_strips,
                ...
                st.success("✅ Store medicine added successfully!")
                st.rerun()

          st.markdown("### 📋 Medical Store Inventory")
          ms_data = run_query(
              "SELECT item_name, strips_count, tablets_per_strip,"
              " purchase_price_strip, selling_price_strip, gst_rate FROM"
              " medical_store WHERE shop_email=?",
              (st.session_state.user_email,),
          )
          if ms_data:
            st.dataframe(
            ...
            st.info("No store medicines added yet.")

      with tab_prof:
        st.subheader("⚙️ Update Shop Profile & Settings")
        up_img = st.file_uploader(
            "Upload Profile Photo", type=["jpg", "png", "jpeg"]
        )
        if up_img and st.button("💾 Save Profile Photo"):
          run_query(
              "UPDATE users SET shop_photo=? WHERE email=?",
              (up_img.read(), st.session_state.user_email),
          )
        ...
        with st.form("shop_profile_form"):
          c1, c2 = st.columns(2)
          with c1:
            raw_upi = st.text_input(
                "Shop UPI ID", value=shop_upi if shop_upi else ""
            )
            new_upi = str(raw_upi).strip()
          with c2:
            raw_gst = st.text_input(
                "Shop GST No.", value=shop_gst if shop_gst else ""
            )
  ...
  if st.session_state.current_page == "Home Ground":
    settings = run_query(
        "SELECT notice_text, home_banner FROM admin_settings WHERE id=1"
    )[0]
    st.markdown(
        f"""<div class="notice-board"><marquee behavior="scroll" direction="left" scrollamount="8">📢 {str(settings[0]).upper() if settings[0] else "WELCOME TO KULU SMART ERP!"}</marquee></div>""",
        unsafe_allow_html=True,
    )
    if settings[1]:
      st.image(settings[1], use_container_width=True)
    else:
      st.markdown(
          """<div class="hero-container"><div class="hero-title">🚀 KULU SMART ERP & POS</div><div class="hero-subtitle">NEXT-GEN CLOUD BILLING, BARCODE & INVENTORY</div></div>""",
          unsafe_allow_html=True,
      )
    ...
    with col1:
      st.markdown(
          """<div class="feature-card border-admin"><div class="card-icon">👑</div><div class="card-title">Super Admin</div><div class="card-text">Control software licensing and global system settings.</div></div>""",
          unsafe_allow_html=True,
      )
    ...
    with col2:
      st.markdown(
          """<div class="feature-card border-wholesale"><div class="card-icon">🏢</div><div class="card-title">Wholesale Hub</div><div class="card-text">Manage massive B2B sales and track retailer network.</div></div>""",
          unsafe_allow_html=True,
      )
    ...
    with col3:
      st.markdown(
          """<div class="feature-card border-shop"><div class="card-icon">🛒</div><div class="card-title">Retail POS</div><div class="card-text">Lightning fast barcode billing & smart inventory tools.</div></div>""",
          unsafe_allow_html=True,
      )
    ...
    st.markdown(
        """<div class="register-section"><h2 style='color: #1a1a1a; font-weight: 800; margin-bottom: 20px;'>READY TO TRANSFORM YOUR BUSINESS?</h2><p style='color: #666; font-size: 18px; margin-bottom: 30px;'>Join thousands of businesses using Kulu Smart ERP today.</p>""",
        unsafe_allow_html=True,
    )
    ...
    with c2:
      ...
      with cc1:
        if st.button(
            "🚀 Buy Software License", use_container_width=True, type="primary"
        ):
    ...
    st.markdown(
        "<div class='footer'>© 2026 Kulu Smart Solutions Global. Engineered for"
        " Excellence.</div>",
        unsafe_allow_html=True,
    )
    ...
    with c_l1:
      if st.button("Login", type="primary"):
        ...
        user = run_query(
            "SELECT name, role, approved, is_deleted FROM users WHERE email=? AND password=?",
            (l_email, hash_attempt),
        )
        if user:
          if user[0][3] == 1:
    ...
    settings = run_query(
        "SELECT upi_id, demo_price, monthly_price, six_month_price,"
        " yearly_price, lifetime_price, soft_gst FROM admin_settings WHERE"
        " id=1"
    )[0]
    packages = {
    ...
    with st.form("reg_form"):
      ...
      r_name = str(raw_r_name).strip().upper()
      raw_r_owner = st.text_input("Owner Name")
      r_owner = str(raw_r_owner).strip().upper()
      ...
      if st.form_submit_button("Next ➡️"):
        if r_name and r_email and r_pass and r_mob:
          ...
          st.warning(
              "⚠️ Please fill all required fields (Business Name, Email,"
              " Password, Mobile)."
          )
    ...
    if st.button("Submit & Verify"):
      if r_utr.strip():
        with st.spinner(
            "⏳ Verifying UTR with Bank Gateway & Generating License Key..."
            " Please wait (30s)"
        ):
        ...
        try:
          run_query(
              """INSERT INTO users (name, owner_name, email, password, role, payment_status, approved, utr_no, paid_amount, package_type, license_key, expiry_date, key_entered, mobile, aadhar, pan, address, state) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
              (
                  d["name"],
                  d["owner"],
                  d["email"],
          ...
          run_query(
              "UPDATE users SET password=?, license_key=?, expiry_date=?,"
              " package_type=?, utr_no=?, paid_amount=?, key_entered=0 WHERE"
              " email=?",
              (
                  hash_new_pass,
                  new_key,
        ...
        if email_sent:
          st.success(
              "✅ Payment Verified Successfully! License Key has been"
              " automatically emailed to your inbox."
          )
        else:
          st.warning(
              "⚠️ Payment verified, but email delivery had a network issue."
              f" Your Manual License Key is: **{new_key}** (Please copy and"
              " save it!)"
          )
        st.balloons()
      else:
    ...
    if "f_step" not in st.session_state:
      st.session_state.f_step = 1
      ...
      if st.button("Send OTP"):
        ...
        if chk:
          ...
          send_real_email(
              f_email,
              "Password Reset OTP",
              f"Your OTP for Kulu ERP Password Reset is: {otp_code}",
          )
          st.success("✅ OTP sent to your registered email!")
          st.session_state.f_step = 2
          st.rerun()
        else:
        ...
        if entered_otp.strip() == str(st.session_state.get("otp_code")):
          if new_pass1 == new_pass2 and new_pass1:
            ...
            run_query(
                ...
                (new_h, st.session_state.get("f_email")),
            )
