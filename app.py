...
if st.session_state.logged_in:
    ...
    if menu == "Logout":
        ...
        # ==============================================================
        # 🟢 MEDICINE WHOLESALE DASHBOARD (USES medical_wholesaler TABLE)
        # ==============================================================
            ...
            with tab_med_entry:
                ...
                with st.form("med_ws_new_form"):
                    ...
                    mw_boxes = st.number_input("Total Boxes / Cartons", min_value=1, value=10)
                    mw_spb = st.number_input("Strips / Packets per Box", min_value=1, value=200)
                    mw_tps = st.number_input("Tablets per Strip", min_value=1, value=10)
                    mw_p_box = st.number_input("Buy Rate per Box (₹)", min_value=0.0, value=1000.0)
                    mw_s_box = st.number_input("Sell Rate per Box (₹)", min_value=0.0, value=1200.0)
                    mw_gst = st.selectbox("GST Slab", [0.0, 5.0, 12.0, 18.0])
                    ...
                    if st.form_submit_button("💾 Save Wholesale Medicine"):
                        if mw_name:
                            run_query("INSERT INTO medical_wholesaler (shop_email, item_name, box_count, strips_per_box, tablets_per_strip, purchase_price_box, selling_price_box, gst_rate, updated_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                                      (st.session_state.user_email, mw_name.strip().upper(), mw_boxes, mw_spb, mw_tps, mw_p_box, mw_s_box, mw_gst, str(date.today())))
        ...
        # ==============================================================
            ...
            with tab_st_entry:
                ...
                with st.form("med_st_new_form"):
                    ...
                    ms_strips = st.number_input("Total Strips", min_value=1, value=50)
                    ms_tps = st.number_input("Tablets per Strip", min_value=1, value=10)
                    ms_p_strip = st.number_input("Buy Rate per Strip (₹)", min_value=0.0, value=40.0)
                    ms_s_strip = st.number_input("Sell Rate per Strip (₹)", min_value=0.0, value=60.0)
                    ms_gst = st.selectbox("GST Slab", [0.0, 5.0, 12.0, 18.0], key="ms_gst_key")
                    ...
                    if st.form_submit_button("💾 Save Store Medicine"):
                        if ms_name:
                            run_query("INSERT INTO medical_store (shop_email, item_name, strips_count, tablets_per_strip, purchase_price_strip, selling_price_strip, gst_rate, updated_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                                      (st.session_state.user_email, ms_name.strip().upper(), ms_strips, ms_tps, ms_p_strip, ms_s_strip, ms_gst, str(date.today())))
                            st.success(f"✅ {ms_name.upper()} added to Medical Store database!")
                            st.rerun()
    ...
    if st.session_state.current_page == "Home Ground":
        ...
        # --- ROW 1: ORIGINAL 3 CARDS (Buttons 1, 2, 3) ---
        col1, col2, col3 = st.columns(3)
        with col1:
        ...
        # --- ROW 2: 2 NEW MEDICINE CARDS (Buttons 4, 5) ---
    ...
    # 🟢 DIRECT EMAIL & PASSWORD LOGIN (NO OTP MESS) 🟢
