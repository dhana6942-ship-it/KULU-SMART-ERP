...
# ==========================================
# 1. DATABASE SETUP (Old Data 100% Safe)
# ==========================================
def init_db():
  ...
  # Keeping this safe so old wholesale data isn't lost
  c.execute("""
        CREATE TABLE IF NOT EXISTS medical_wholesaler (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            shop_email TEXT, item_name TEXT, box_count INTEGER, strips_per_box INTEGER, tablets_per_strip INTEGER, purchase_price_box REAL, selling_price_box REAL, gst_rate REAL, barcode TEXT, updated_date TEXT
        )
    """)

  cols_to_add = [
      ("utr_no", "TEXT"), ("paid_amount", "REAL"), ("package_type", "TEXT"),
      ("license_key", "TEXT"), ("is_deleted", "INTEGER DEFAULT 0"),
      ("shop_photo", "BLOB"), ("expiry_date", "TEXT"), ("key_entered", "INTEGER DEFAULT 0"),
      ("upi_id", "TEXT"), ("owner_name", "TEXT"), ("pan_gst_no", "TEXT"),
      ("address", "TEXT"), ("state", "TEXT"), ("aadhar", "TEXT"), ("pan", "TEXT"),
      ("gst", "TEXT"), ("mobile", "TEXT"),
  ]
  ...
  tx_cols = [
      ...
      ("invoice_no", "TEXT"), ("rate", "REAL"), ("gst_pct", "REAL"),
  ...
  c.execute(
      "INSERT OR IGNORE INTO users (name, email, password, role, payment_status, approved, is_deleted) VALUES (?, ?, ?, ?, ?, ?, ?)",
      ("Super Admin", "dhana6942@gmail.com", admin_hash, "SuperAdmin", "Paid", 1, 0),
  )
  try:
    c.execute(
        "UPDATE users SET role='SuperAdmin', password=?, approved=1, payment_status='Paid', is_deleted=0 WHERE email='dhana6942@gmail.com'",
        (admin_hash,),
    )
  except:
    pass
  c.execute(
      "INSERT OR IGNORE INTO admin_settings (id, upi_id, monthly_price, yearly_price, lifetime_price, soft_gst) VALUES (1, 'kulusutar@ybl', 499, 4999, 9999, 18)"
  )
  conn.commit()
  conn.close()
...
def generate_receipt_html(
    shop_name, item_name, qty, rate, gst_pct, gst_amt, total_price, date_str, shop_upi="", cust_name="", cust_mob="", inv_no="", base_amt=0,
):
  qr_html = ""
  if shop_upi:
...
INDIAN_STATES = ["Andhra Pradesh", "Odisha", "Maharashtra", "Karnataka", "West Bengal", "Gujarat", "Delhi"]
...
if st.session_state.logged_in:
  ...
  if menu == "Logout":
    ...
    if st.session_state.user_role == "SuperAdmin":
        ...
        if active:
          ...
          with c1:
            ...
            if st.button("📧 Manual Resend Mail"):
              ...
              if send_real_email(sel_mail, f"Your Kulu ERP {usr[3]} License", f"Here is your requested License Key.\n🔑 License Key: {usr[5]}\n📅 Expiry Date: {usr[4]}\n"):
      ...
      with tab_prof:
        ...
        except Exception as e: st.error("Backup file not found.")
      ...
      # Grocery logic uses 'inventory' table, Medicine logic uses 'medical_store' table.
      if st.session_state.user_role in ["Grocery", "Shop"]:
        tab_dash, tab_stock, tab_purch, tab_sales, tab_hist, tab_gst_rep, tab_prof = st.tabs(["📈 Dash", "📦 Grocery Stock", "📥 Grocery Purchase", "🧾 Grocery POS", "🖨️ History", "📊 GST Reports", "⚙️ Settings"])
        ...
        with tab_stock:
          ...
          if inv_data:
            ...
            st.markdown("---")
            st.dataframe(pd.DataFrame(stock_list, columns=["Product Name", "Today IN", "Today OUT", "Current Stock", "Buy Rate (₹)", "Stock Value (₹)"]), use_container_width=True)
          else: st.info("No stock data.")

        with tab_purch:
          st.subheader("📥 Add Grocery Inventory (Purchase Entry)")
          i_bcode = st.text_input("||||| Scan Barcode Here 👇", key="p_bcode")
          ...
          def_name = str(existing_item[0][0]).upper() if existing_item else ""
          ...
          with c1: raw_i_name = st.text_input("Product Name", value=def_name, key="p_name"); i_name = str(raw_i_name).strip().upper()
          ...
          with c4:
            i_pprice = st.number_input("Buy Rate (₹)", min_value=0.0, value=def_buy, step=10.0, key="p_pprice")
          ...
          if st.button("💾 Save Purchase", use_container_width=True, type="primary") and i_name:
      ...
      elif st.session_state.user_role in ["Medicine", "Wholesaler"]:
        tab_dash, tab_stock, tab_purch, tab_sales, tab_hist, tab_gst_rep, tab_prof = st.tabs(["📈 Dash", "💊 Medicine Stock", "📥 Medicine Purchase", "🧾 Pharmacy POS", "🖨️ History", "📊 GST Reports", "⚙️ Settings"])
        ...
        with tab_stock:
          st.subheader("📦 Live Medicine Register (Strips)")
          inv_data = run_query("SELECT item_name, strips_count, purchase_price_strip, selling_price_strip, tablets_per_strip FROM medical_store WHERE shop_email=?", (st.session_state.user_email,))
          sales_today = run_query("SELECT item_name, SUM(qty) FROM transactions WHERE shop_email=? AND date=? AND trans_type='Sale' GROUP BY item_name", (st.session_state.user_email, today_str))
          sales_dict = {row[0]: row[1] for row in sales_today}
          purchases_today = run_query("SELECT item_name, SUM(qty) FROM transactions WHERE shop_email=? AND date=? AND trans_type='Purchase' GROUP BY item_name", (st.session_state.user_email, today_str))
          purchases_dict = {row[0]: row[1] for row in purchases_today}
          ...
          if inv_data:
            for row in inv_data:
              i_name = str(row[0]).upper(); curr_stock = row[1]; buy_price = row[2]; sell_price = row[3]; tab_strip = row[4]
              stock_in = purchases_dict.get(row[0], 0); stock_out = sales_dict.get(row[0], 0)
              stock_list.append([i_name, stock_in, stock_out, curr_stock, tab_strip, buy_price, sell_price])
            st.dataframe(pd.DataFrame(stock_list, columns=["Medicine Name", "Today IN (Strips)", "Today OUT (Strips)", "Current Stock (Strips)", "Tablets/Strip", "Buy/Strip (₹)", "Sell/Strip (₹)"]), use_container_width=True)
          else: st.info("No medicine stock data.")

        with tab_purch:
          st.subheader("📥 Add Medicine Inventory")
          i_bcode = st.text_input("||||| Scan Barcode Here 👇", key="p_bcode_med")
          existing_item = run_query("SELECT item_name, purchase_price_strip, selling_price_strip, tablets_per_strip FROM medical_store WHERE barcode=? AND shop_email=?", (i_bcode, st.session_state.user_email)) if i_bcode else []
          def_name = str(existing_item[0][0]).upper() if existing_item else ""
          def_buy = float(existing_item[0][1]) if existing_item else 0.0
          def_sell = float(existing_item[0][2]) if existing_item else 0.0
          def_tab = int(existing_item[0][3]) if existing_item else 10
          if existing_item: st.success(f"Medicine found: {def_name}. Enter quantity to purchase!")

          c1, c2, c3, c4 = st.columns([2, 1, 1, 1])
          with c1: ms_name = st.text_input("Medicine Name", value=def_name, key="p_name_med")
          with c2: ms_strips = st.number_input("Strips Purchased", min_value=1, value=10, key="p_qty_med")
          with c3: ms_t_strip = st.number_input("Tablets per Strip", min_value=1, value=def_tab)
          with c4:
            gst_options = {"No GST (0%)": 0, "5% GST": 5, "12% GST": 12, "18% GST": 18, "28% GST": 28}
            ms_gst_pct = gst_options[st.selectbox("Purchase GST Slab", list(gst_options.keys()), key="p_gst_med")]
          
          c5, c6 = st.columns(2)
          with c5: ms_p_strip = st.number_input("Buy Rate Base per Strip(₹)", min_value=0.0, value=def_buy)
          with c6: ms_s_strip = st.number_input("Sell Rate per Strip(₹)", min_value=0.0, value=def_sell if def_sell>0 else ms_p_strip*1.2)

          if st.button("💾 Save Medicine Purchase", use_container_width=True, type="primary") and ms_name:
            p_base = ms_p_strip * ms_strips
            p_gst_amt = (p_base * ms_gst_pct) / 100
            p_final = p_base + p_gst_amt
            if existing_item: run_query("UPDATE medical_store SET strips_count = strips_count + ?, purchase_price_strip=?, selling_price_strip=? WHERE barcode=? AND shop_email=?", (ms_strips, ms_p_strip, ms_s_strip, i_bcode, st.session_state.user_email))
            else: run_query("INSERT INTO medical_store (shop_email, item_name, strips_count, tablets_per_strip, purchase_price_strip, selling_price_strip, gst_rate, barcode, updated_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (st.session_state.user_email, ms_name.strip().upper(), ms_strips, ms_t_strip, ms_p_strip, ms_s_strip, ms_gst_pct, i_bcode, str(date.today())))
            run_query("""INSERT INTO transactions (shop_email, date, item_name, qty, total_price, profit, is_gst, trans_type, rate, gst_pct, gst_amt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""", (st.session_state.user_email, str(date.today()), ms_name.strip().upper(), ms_strips, p_final, 0, 1 if ms_gst_pct > 0 else 0, "Purchase", ms_p_strip, ms_gst_pct, p_gst_amt))
            st.success(f"✅ Purchase Saved! Value: ₹{p_final:.2f}")

        with tab_sales:
          st.subheader("🧾 Pharmacy POS (Strips & Tablets)")
          if "print_receipt_med" in st.session_state:
            st.success("✅ Sale Recorded Successfully!")
            components.html(st.session_state.print_receipt_med, height=600)
            if st.button("➕ Create New Bill", type="primary"): del st.session_state.print_receipt_med; st.rerun()
          else:
            with st.expander("👤 Customer Details", expanded=False):
              c1, c2 = st.columns(2)
              with c1: raw_c_name = st.text_input("Customer Name", key="cname_med"); cust_name = str(raw_c_name).strip().upper()
              with c2: cust_mob = st.text_input("Mobile Number", key="cmob_med")

            scan_code = st.text_input("🔍 SCAN BARCODE HERE...", key="s_scan_med")
            stock_items_pos = run_query("SELECT id, item_name, selling_price_strip, strips_count, purchase_price_strip, gst_rate, barcode, tablets_per_strip FROM medical_store WHERE shop_email=? AND strips_count > 0", (st.session_state.user_email,))
            if stock_items_pos:
              item_dict_pos = {f"{str(item[1]).upper()} - ₹{item[2]}/Strip (Strips: {item[3]})": item for item in stock_items_pos}
              default_index_pos = 0
              if scan_code:
                for idx, item in enumerate(stock_items_pos):
                  if str(item[6]) == str(scan_code): default_index_pos = idx; st.success(f"Barcode Matched: {str(item[1]).upper()}"); break
              sel_item_pos = st.selectbox("Select Medicine", list(item_dict_pos.keys()), index=default_index_pos)
              i_id, i_name_raw, default_sprice, i_stock, i_pprice, def_gst, _, t_per_s = item_dict_pos[sel_item_pos]
              i_name = str(i_name_raw).upper()

              b_mode = st.radio("Unit Mode", ["Strips / Packets", "Individual Tablets"])
              col1, col2, col3 = st.columns(3)
              
              if b_mode == "Strips / Packets":
                with col1: s_qty = st.number_input("Number of Strips", min_value=1, value=1)
                with col2: s_price = st.number_input("Rate per Strip (₹)", value=float(default_sprice))
                deduct_val = s_qty
              else:
                p_tab = float(default_sprice) / int(t_per_s) if t_per_s > 0 else float(default_sprice)
                with col1: s_qty = st.number_input("Number of Tablets", min_value=1, value=1)
                with col2: s_price = st.number_input("Rate per Tablet (₹)", value=float(p_tab))
                deduct_val = s_qty / int(t_per_s)

              with col3: 
                gst_options = {"No GST (0%)": 0, "5% GST": 5, "12% GST": 12, "18% GST": 18, "28% GST": 28}
                s_gst_pct = gst_options[st.selectbox("Select GST Slab", list(gst_options.keys()), key="sgst_med")]

              s_base = s_price * s_qty
              if s_gst_pct > 0: gst_amt = (s_base * s_gst_pct) / 100; s_final_price = s_base + gst_amt; is_gst_bill = 1
              else: gst_amt = 0; s_final_price = s_base; is_gst_bill = 0
              profit = s_final_price - ((i_pprice/t_per_s if b_mode=="Individual Tablets" else i_pprice) * s_qty)
              st.write(f"**Total Payable:** ₹ {s_final_price:.2f}")

              if st.button("🛒 Generate Medicine Bill", type="primary"):
                if deduct_val <= i_stock:
                  inv_no = "INV-" + "".join(random.choices(string.digits, k=6))
                  run_query("UPDATE medical_store SET strips_count = strips_count - ? WHERE id=?", (deduct_val, i_id))
                  run_query("""INSERT INTO transactions (shop_email, date, item_name, qty, total_price, profit, is_gst, trans_type, customer_name, customer_mobile, invoice_no, rate, gst_pct, gst_amt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""", (st.session_state.user_email, str(date.today()), i_name, s_qty, s_final_price, profit, is_gst_bill, "Sale", cust_name, cust_mob, inv_no, s_price, s_gst_pct, gst_amt))
                  st.session_state.print_receipt_med = generate_receipt_html(shop_name, i_name, s_qty, s_price, s_gst_pct, gst_amt, s_final_price, str(date.today()), shop_upi, cust_name, cust_mob, inv_no, s_base); st.rerun()
                else: st.error("Not enough stock!")

      # HISTORY & GST REPORTS FOR BOTH
      with tab_hist:
  ...
  if st.session_state.current_page == "Home Ground":
    ...
    # 🟢 EXACTLY 2 BOXES ON HOME PAGE AS REQUESTED 🟢
    col1, col2 = st.columns(2)
    with col1:
      st.markdown("""<div class="feature-card border-shop"><div class="card-icon">🛒</div><div class="card-title">Grocery Store</div><div class="card-text">Manage grocery inventory, barcode billing & sales.</div></div>""", unsafe_allow_html=True)
      if st.button("Grocery Login", use_container_width=True):
        st.session_state.current_page = "Login"
        st.session_state.login_role = "Grocery"
        st.rerun()
    with col2:
      st.markdown("""<div class="feature-card border-admin"><div class="card-icon">💊</div><div class="card-title">Medicine Store</div><div class="card-text">Manage medicine strips, tablets & pharmacy POS.</div></div>""", unsafe_allow_html=True)
      if st.button("Medicine Login", use_container_width=True):
        st.session_state.current_page = "Login"
        st.session_state.login_role = "Medicine"
        st.rerun()
        
    st.markdown("<br><div style='text-align: center;'>", unsafe_allow_html=True)
    if st.button("🛡️ Super Admin Login"):
        st.session_state.current_page = "Login"
    ...
    with c_l1:
      if st.button("Login", type="primary"):
        ...
        if user:
          ...
          # Safely allowing old "Shop" users into Grocery, and "Wholesaler" into Medicine
          elif user[0][1] == st.session_state.login_role or (st.session_state.login_role == "Grocery" and user[0][1] == "Shop") or (st.session_state.login_role == "Medicine" and user[0][1] == "Wholesaler"):
            st.session_state.logged_in = True
            st.session_state.user_role = user[0][1] if user[0][1] in ["Shop", "Wholesaler"] else st.session_state.login_role
    ...
    with st.form("reg_form"):
      r_role = st.selectbox("Register As", ["Grocery", "Medicine"])
      ...
      if st.form_submit_button("Next ➡️"):
        if r_name and r_email and r_pass and r_mob:
          ...
          st.rerun()
        else: st.warning("⚠️ Please fill all required fields (Business Name, Email, Password, Mobile).")
    ...
    if st.button("Submit & Verify"):
      if r_utr.strip():
        ...
        if email_sent: st.success("✅ Payment Verified Successfully! License Key has been automatically emailed to your inbox.")
        else: st.warning(f"⚠️ Payment verified, but email delivery had a network issue. Your Manual License Key is: **{new_key}** (Please copy and save it!)")
        st.balloons()
      else: st.error("⚠️ Please enter a valid 12-Digit UTR No.")
    ...
    if st.session_state.f_step == 1:
      ...
      if st.button("Reset Password", type="primary"):
        if entered_otp.strip() == str(st.session_state.get("otp_code")):
          ...
          else: st.error("❌ Passwords do not match or empty.")
